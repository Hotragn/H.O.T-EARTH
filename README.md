...
- [x] **Meteor showers.** 37 real showers (IAU Meteor Data Center / IMO) with radiants plotted on the celestial sphere, a year calendar, and a "tonight / next up" panel — knitting three tabs together: each parent body links to the Comets & Asteroids tab (Perseids → Swift-Tuttle, Geminids → asteroid Phaethon, Orionids → Halley), and the moon phase at each peak comes from the Moon tab so you know if moonlight will wash it out. ZHR is labeled as the idealized rate; a real altitude-corrected estimate sits beside it. *(this release)*
