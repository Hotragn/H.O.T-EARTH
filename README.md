...
- [x] **Night sky (Beyond).** A real celestial sphere: 9,029 stars from the HYG catalog (Hipparcos/Gaia), sized by measured magnitude and colored from their measured B–V index, with the 88 constellation figures (a labeled cultural overlay — the stars are real, the lines are ours), 110 Messier deep-sky objects, and the ESO Milky Way panorama rotated into equatorial coordinates so the band lines up with the real star density. A **"sky from your location, right now"** planetarium mode uses the same real observer geometry (local sidereal time) that drives every terminator in the app. *(this release)*
